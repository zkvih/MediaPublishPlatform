...
class BaseFileUploader(object):
    ...
    def __init__(
        ...
        self.logger = create_logger(
            self.platform_name, f"logs/{self.platform_name}.log"
        )
        # 是否跳过验证cookie有效性
    ...
    def _resolve_executable_path(self, executable_path):
        ...
        if not os.path.exists(candidate):
            self.logger.warning(f"浏览器路径不存在，回退默认浏览器: {candidate}")
            return None
        if os.path.isdir(candidate):
            self.logger.warning(f"浏览器路径是目录，回退默认浏览器: {candidate}")
            return None
        if os.name != "nt" and not os.access(candidate, os.X_OK):
            self.logger.warning(f"浏览器路径不可执行，回退默认浏览器: {candidate}")
            return None
        return candidate
    ...
    async def main(self):
        ...
        # .mp4,.mov,.flv,.f4v,.mkv,.rm,.rmvb,.m4v,.mpg,.mpeg,.ts 为视频文件
            ...
            self.logger.error(
                f"{self.platform_name}该文件类型暂不支持：{self.file_path.name}"
            )
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        try:
            ...
            # step1.创建浏览器实例
            executable_path = self._resolve_executable_path(self.local_executable_path)
            if executable_path:
            ...
            if self.file_type == 1:
                await page.goto(
                    self.creator_image_url,
                    wait_until="domcontentloaded",
                    timeout=self.page_load_timeout,
                )
            else:
                await page.goto(
                    self.creator_video_url,
                    wait_until="domcontentloaded",
                    timeout=self.page_load_timeout,
                )
            ...
            await asyncio.sleep(
                self.check_interval
            )  # close delay for look the video status
    ...
    async def handle_instagram_login(self, page):
        ...
        if "loginpage" in current_url or "login" in current_url:
            ...
            try:
                ...
                if login_button:
                    self.logger.info(
                        f"[+]找到Instagram登录按钮，选择器: {found_selector}"
                    )
    ...
    async def find_button(self, selector_list):
        ...
        for selector in selector_list:
            ...
            if count > 0:
                # 异步等待元素可交互（避免元素未加载完成）
                await self.locator_base.locator(selector).wait_for(
                    state="visible", timeout=self.button_visible_timeout
                )
                self.logger.info(
                    f"找到按钮定位器: {selector}, 是否可见: {await self.locator_base.locator(selector).is_visible()}"
                )
                # 返回找到的按钮定位器
        ...
        while True:
            try:
                ...
                if self.platform_name == "kuaishou":
                    ...
                    publish_button = await self.find_button(
                        self.publish_button_selectors
                    )
                    # 检查发布按钮是否可点击
                    if (
                        publish_button
                        and await publish_button.get_attribute("disabled") is None
                    ):
                        ...
                        if error_element:
                            self.logger.info(
                                "  [-] found error while uploading now retry..."
                            )
    ...
    async def add_title_tags(self, page):
        ...
        try:
            ...
            if self.title_supported and self.title:
                ...
                self.logger.info(
                    f"  [-] 将点击标题输入框: {await editor_button.text_content()}"
                )
            ...
            if self.textbox_supported and self.text:
                ...
                if textbox_button:
                    self.logger.info(
                        f"  [-] 将点击正文输入框: {await textbox_button.text_content()}"
                    )
    ...
    async def set_thumbnail(self, page):
        ...
        if self.thumbnail_path:
            self.logger.info(
                f"  [-] 将点击封面选择按钮: {await self.find_button(self.thumbnail_button_selectors).text_content()}"
            )
            await self.find_button(self.thumbnail_button_selectors).click()
            self.logger.info(
                f"  [-] 将点击封面上传按钮: {await self.find_button(self.thumbnail_upload_selectors).text_content()}"
            )
            await self.find_button(self.thumbnail_upload_selectors).set_input_files(
                self.thumbnail_path
            )
            await page.wait_for_timeout(2000)  # 等待2秒
            self.logger.info(
                f"  [-] 将点击封面确认按钮: {await self.find_button(self.thumbnail_finish_selectors).text_content()}"
            )
            await self.find_button(self.thumbnail_finish_selectors).click()
            self.logger.info(
                f"  [-] 将点击封面关闭按钮: {await self.find_button(self.thumbnail_close_selectors).text_content()}"
            )
    ...
    async def set_schedule_time(self, page, publish_date):
        ...
        self.logger.info(
            f"  [-] 将点击定时发布按钮: {await schedule_button.text_content()}"
        )
        ...
        # 设置日期和时间
        await page.fill(
            '[aria-label="Date"]', publish_datetime.strftime(self.date_format)
        )
        await page.fill(
            '[aria-label="Time"]', publish_datetime.strftime(self.time_format)
        )
            ...
            try:
                ...
                if publish_button:
                    ...
                    if self.platform_name == "tiktok":
                        ...
                        try:
                            await page.wait_for_selector(
                                'button.TUXButton.TUXButton--primary div.TUXButton-label:has-text("Post now")',
                                timeout=5000,
                            )
                            self.logger.info(
                                "  [-]检测到版权检查弹窗，准备点击Post now按钮"
                            )
                            # 使用更精确的选择器点击Post now按钮
                            await self.locator_base.locator(
                                "button.TUXButton.TUXButton--primary div.TUXButton-label >> text=Post now"
                            ).click()
                ...
                if upload_button:
                    self.logger.info(
                        f"发布尝试 {attempt}，上传按钮可见状态: {await upload_button.is_visible()}"
                    )
                    await upload_button.wait_for(
                        state="visible", timeout=self.button_visible_timeout
                    )
    ...
    async def platform_setup(self, handle=False):
        ...
        if not os.path.exists(account_file) or not await self.cookie_auth():
            ...
            await self.get_platform_cookie(
                account_file,
    ...
    async def cookie_auth(self):
        ...
        try:
            ...
            cookie_file_path = os.path.relpath(
                self.account_file, os.path.join(BASE_DIR, "cookiesFile")
            )
    ...
    async def get_platform_cookie(
        ...
        """
        async with async_playwright() as playwright:
            launch_args = [f"--lang {browser_lang}"]
            resolved_executable_path = self._resolve_executable_path(executable_path)
            ...
            page = await context.new_page()
            await page.goto(
                self.login_url, wait_until="domcontentloaded", timeout=timeout
            )
            await page.pause()
            # 等待用户登录完成
